...
# load cams for .npy
root_dir = 'myExperiment/results/EM_rounds_prostate/round_1/prediction'
# cam_dir = os.path.join(root_dir, 'cam')
lpcam_dir = os.path.join(root_dir, 'volume_rw')
lpcam_save_dir =create_directory(os.path.join(root_dir, 'volume_rw_post'))
...
for volume_id in volume_list:
    ...
    infer_list = "data/prostate/train_valid_prostate_v2_" + str(volume_id).zfill(4) + ".txt"
    ...
    lpcam_0_dict = np.load(os.path.join(lpcam_dir, img_name_list[0] + '.npy'), allow_pickle=True)
    # lpcam_0 = lpcam_0_dict['high_res'].numpy()
    lpcam_0 = lpcam_0_dict
    ...
    for img_name in img_name_list:
        ...
        lpcam_dict = np.load(os.path.join(lpcam_dir, img_name + '.npy'), allow_pickle=True)
    ...
    for i in range(len(dict_pool)):
        ...
        if mean_diff > 0.04:
            dict_pool[i]["tag"] = 0
            print(f'{dict_pool[i]["name"]}: {mean_diff}')
    ...
    for i in range(round(len(dict_pool)/2), -1, -1):
        ...
        if dict_pool[i]["tag"]==0:
            ...
            lpcam_dict = np.load(os.path.join(lpcam_dir, dict_pool[i]["name"] + '.npy'), allow_pickle=True)
            # lpcam_dict['high_res'] = torch.tensor(dict_pool[i]["mat"])
            lpcam_dict = torch.tensor(dict_pool[i]["mat"])
