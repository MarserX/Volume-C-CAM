...
def save_feature(save_dir,ckpt_path,prostate_root): ####### save feature from resnet50 at 'cluster/cam_feature/'
    model = getattr(importlib.import_module("net.resnet50_cam"), 'Net_CAM')()
    model.load_state_dict(torch.load((ckpt_path)), strict=True)
    model.eval()
    model.cuda()

    infer_dataset = data.dataloader_prostate.ProstateClassificationDataset("data/train_valid_prostate_v2.txt", prostate_root=prostate_root,
...
def make_lpcam(args,workspace,lpcam_out_dir,ckpt_path,prostate_root,list_name='voc12/train.txt'):
...
def run(args):
    ...
    # save_feature(save_dir=os.path.join(args.round_out_dir,'cam_feature'),ckpt_path=os.path.join(args.round_out_dir, 'checkpoints', args.cam_weights_name),prostate_root=args.data_root)
    ...
    make_lpcam(args,workspace=args.round_out_dir,lpcam_out_dir=lpcam_out_dir_,ckpt_path=ckpt_path_,prostate_root=args.data_root,list_name=args.infer_list)
