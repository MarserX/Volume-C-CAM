...
def make_volume_cam(args, model):
    ...
    infer_dataset = data.dataloader_prostate.ProstateClassificationDatasetTx(args.infer_list, prostate_root=args.data_root,
                                                                           phase='train', label_dir=args.seg_pred_dir)
    ...
    if not args.load_feature:
        with torch.no_grad():
            for i, pack in enumerate(infer_data_loader):
                ...
                seg_pred = pack['seg_pred'].type(torch.float32).cuda(non_blocking=True)

                x, cams, feature = model(img, seg_pred.unsqueeze(1))
    ...
    if args.load_cluster:
        ...
        mask_dir = os.path.join(
            args.round_out_dir, 'prediction', args.cam_out_dir)
        for idx in img_selected:
            ...
            for i in range(len(valid_cat)):
                if valid_cat[i] == class_id:
                    mask = mask[i]
                    fg_position_selected = (mask) > 0.5
                    bg_position_selected = (mask) < 0.3
    ...
    with torch.no_grad():
        for i, pack in enumerate(tqdm(data_loader)):
            ...
            features = []
            for (img, seg_pred) in zip(imgs, seg_preds):
                _, _, feature = model(img[0].cuda(), seg_pred[0].unsqueeze(1).type(torch.float32).cuda())
...
if __name__ == "__main__":
    ...
    pyutils.Logger(args.log_name + '_make_volume_cam_round1.log')
    print(vars(args))
    ...
    ckpt_path = ckpt_path = os.path.join(
        args.round_out_dir, 'checkpoints', args.cam_weights_name)
    model = getattr(importlib.import_module(args.cam_network), 'Net_CAM')(DIC_FILE)
    model.load_state_dict(torch.load((ckpt_path)), strict=True)
    model.eval()
    model.cuda()
