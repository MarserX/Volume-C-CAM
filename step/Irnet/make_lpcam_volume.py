...
def save_feature(save_dir,ckpt_path,prostate_root,volume_id): ####### save feature from resnet50 at 'cluster/cam_feature/'
    model = getattr(importlib.import_module("net.resnet50_cam"), 'Net_CAM')()
    model.load_state_dict(torch.load((ckpt_path)), strict=True)
    model.eval()
    model.cuda()

    infer_dataset = data.dataloader_prostate.ProstateClassificationDataset(args.infer_list, prostate_root=prostate_root,
                                                                phase='train')
    ...
    torch.save(tensor_logits, os.path.join(save_dir,'tensor_logits_'+str(volume_id).zfill(4)+'.pt'))
    torch.save(tensor_feature,os.path.join(save_dir,'tensor_feature_'+str(volume_id).zfill(4)+'.pt'))
    torch.save(tensor_label,os.path.join(save_dir,'tensor_label_'+str(volume_id).zfill(4)+'.pt'))
    np.save(os.path.join(save_dir,'name2id_'+str(volume_id).zfill(4)+'.npy'), name2id)

def load_feature_select_and_cluster(workspace, feature_dir, mask_dir, ckpt_path, volume_id, load_cluster=False, num_cluster=5,select_thres=0.1,class_thres=0.9,context_thres=0.9,context_thres_low=0.05,tol=5):
    tensor_feature = torch.load(os.path.join(feature_dir,'tensor_feature_'+str(volume_id).zfill(4)+'.pt'))
    tensor_label = torch.load(os.path.join(feature_dir,'tensor_label_'+str(volume_id).zfill(4)+'.pt'))
    name2id = np.load(os.path.join(feature_dir,'name2id_'+str(volume_id).zfill(4)+'.npy'), allow_pickle=True).item()
    tensor_logits = torch.load(os.path.join(feature_dir,'tensor_logits_'+str(volume_id).zfill(4)+'.pt'))

    # load confounder
    confounder_dict = np.load(os.path.join(workspace, 'confounder' + '.npy'), allow_pickle=True)
    ...
    for class_id in range(1):
        ...
        if load_cluster:
            cluster_centers = torch.load(os.path.join(cluster_result_dir,'cluster_centers_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
            cluster_centers2 = torch.load(os.path.join(cluster_result_dir,'cluster_centers2_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
            cluster_ids_x = torch.load(os.path.join(cluster_result_dir,'cluster_ids_x_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
            cluster_ids_x2 = torch.load(os.path.join(cluster_result_dir,'cluster_ids_x2_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
        else:
            ...
            for idx in img_selected:
                ...
                for i in range(len(valid_cat)):
                    if valid_cat[i]==class_id:
                        mask = mask[i]
                        position_selected = (mask)>0.7
                        position_not_selected = (mask)<0.1
            ...
            cluster_ids_x, cluster_centers = kmeans(X=feature_selected, num_clusters=5, distance='cosine', device=torch.device('cuda:0'), tol=tol)
            cluster_ids_x2, cluster_centers2 = kmeans(X=feature_not_selected, num_clusters=5, distance='cosine', device=torch.device('cuda:0'), tol=tol)

            cluster_centers_m = feature_selected.mean(keepdim=True, dim=0)
            cluster_centers2_m = feature_not_selected.mean(keepdim=True, dim=0)
        
            torch.save(cluster_centers.cpu(), os.path.join(cluster_result_dir,'cluster_centers_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
            torch.save(cluster_centers2.cpu(), os.path.join(cluster_result_dir,'cluster_centers2_'+str(class_id)+'_'+str(volume_id).zfill(4)+'.pt'))
            # torch.save(cluster_ids_x.cpu(), os.path.join(cluster_result_dir,'cluster_ids_x_'+str(class_id)+'_0140.pt'))
            # torch.save(cluster_ids_x2.cpu(), os.path.join(cluster_result_dir,'cluster_ids_x2_'+str(class_id)+'_0140.pt'))
    ...
    os.makedirs(os.path.join(workspace, 'selected_cluster'),exist_ok=True)
    # torch.save(centers.cpu(), osp.join(workspace+'class_ceneters'+'.pt'))
    torch.save(centers, os.path.join(workspace,'selected_cluster/class_ceneters'+'_'+str(volume_id).zfill(4)+'.pt'))
    torch.save(context, os.path.join(workspace,'selected_cluster/class_context'+'_'+str(volume_id).zfill(4)+'.pt'))

def make_lpcam(cam_network,round_out_dir,cam_weights_name,infer_list,data_root,cam_scales,workspace,lpcam_out_dir,volume_id):
    cluster_centers = torch.load(os.path.join(workspace,'selected_cluster/class_ceneters'+'_'+str(volume_id).zfill(4)+'.pt'))
    cluster_context = torch.load(os.path.join(workspace,'selected_cluster/class_context'+'_'+str(volume_id).zfill(4)+'.pt'))

    # cluster_centers_ = torch.load(os.path.join(workspace,'class_ceneters'+'.pt'))
    # cluster_context_ = torch.load(os.path.join(workspace,'class_context'+'.pt'))
    
    model = getattr(importlib.import_module(cam_network), 'Net_CAM')()
    model.load_state_dict(torch.load(os.path.join(round_out_dir, 'checkpoints', cam_weights_name)), strict=True)
    model.eval()
    model.cuda()

    dataset = data.dataloader_prostate.ProstateClassificationDatasetMSF(infer_list,
                                                             prostate_root=data_root, scales=cam_scales)
    data_loader = DataLoader(dataset, shuffle=False, num_workers=18 // 1, pin_memory=False)
            ...
            for class_id in valid_cat:
                ...
                for feature in features:
                    ...
                    for j in range(cluster_feature.shape[0]): 
                        ...
                        cluster_feature_here = cluster_feature[j].cuda()
                    ...
                    if context_feature.shape[0]>0:
                        ...
                        for j in range(context_feature.shape[0]):
                            ...
                            # context_attmap = torch.matmul(feature_here,context_feature_here).unsqueeze(0).unsqueeze(0)
                            context_attmap = torch.mean(feature_here,-1).unsqueeze(0).unsqueeze(0)
...
def run(i):
    ...
    # save_feature(save_dir=os.path.join(args.round_out_dir,'cam_feature'),ckpt_path=os.path.join(args.round_out_dir, 'checkpoints', args.cam_weights_name),prostate_root=args.data_root,volume_id=i)
    # load_feature_select_and_cluster(args.round_out_dir,feature_dir=feature_dir_, mask_dir=mask_dir_, ckpt_path=ckpt_path_, select_thres=0.5,volume_id=i)
    make_lpcam(cam_network=args.cam_network,
               round_out_dir=args.round_out_dir, 
               cam_weights_name=args.cam_weights_name,
               infer_list=args.infer_list,
               data_root=args.data_root,
               cam_scales=args.cam_scales,
               workspace=args.round_out_dir,
               lpcam_out_dir=lpcam_out_dir_,
               volume_id=i)
...
if __name__ == "__main__":
    ...
    pyutils.Logger(args.log_name + '_make_lpcam_volume.log')
    print(vars(args))
    ...
    for i in volume_list:
        args.infer_list = "data/train_valid_prostate_v2_" + str(i).zfill(4) + ".txt"
        print("volume_"+ str(i).zfill(4))
        run(i)
    # args.infer_list = "data/train_valid_prostate_v2_0140.txt"
    # run(args)