...
def pil_resize(img, size, order):
    ...
    return np.asarray(Image.fromarray(img.astype(np.uint8)).resize(size[::-1], resample))
...
def random_resize_long(img, min_long, max_long):
    ...
    if isinstance(img, tuple):
        return (pil_rescale(img[0], scale, 3), pil_rescale(img[1], scale, 3))
    else:
        return pil_rescale(img, scale, 3)
...
def crf_inference_label(img, labels, t=10, n_labels=21, gt_prob=0.7):
    ...
    d.addPairwiseBilateral(sxy=50/5, srgb=5/5, rgbim=np.ascontiguousarray(np.copy(img)), compat=10)
